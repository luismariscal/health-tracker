...
class MainActivity : ComponentActivity() {
    ...
    private val trustedAppUri: Uri by lazy { Uri.parse(BuildConfig.WEB_APP_URL) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        webView.settings.userAgentString =
            "${webView.settings.userAgentString} TP-AndroidShell/${BuildConfig.VERSION_NAME}"
        ...
        // Review note: this shell only loads the hosted tracker origin, which
        // keeps the legacy JS interface within trusted content instead of
        // exposing it to arbitrary web navigation.
        val bridge = HealthConnectBridge(this)
        webView.addJavascriptInterface(bridge, "HealthTrackerAndroid")
        webView.addJavascriptInterface(bridge, "AndroidHealthConnect")
    ...
    fun requestHealthConnectSyncJson(days: Int, interactive: Boolean): String {
        val requestId = "sync-${System.currentTimeMillis()}"
        startHealthConnectSync(requestId, days.coerceIn(1, 30), interactive)
        return JSONObject()
            .put("type", "sync-request")
            .put("ok", true)
            .put("requestId", requestId)
            .put("message", "Android Health Connect sync requested.")
            .toString()
    }
        ...
        runCatching {
            ...
            dispatchPayload(
                buildStatusPayload(
                    ...
                    put("ok", false)
                    put("message", error.message ?: "Health Connect sync failed.")
                },
            )
        }
    }
    ...
    private fun buildStatusPayload(
        ...
        val grantedNow = runCatching {
            if (availability == HealthConnectClient.SDK_AVAILABLE) {
                // Review note: the page asks for live shell state, so the shell
                // reads granted permissions on demand instead of caching them.
                runBlocking {
                    HealthConnectClient
    ...
    private fun dispatchPayload(payload: JSONObject) {
        ...
        val js = """
            (function() {
                var payload = $raw;
                if (window.__healthConnectBridgeReceive) {
                    window.__healthConnectBridgeReceive(payload);
                    return;
                }
                if (window.receiveHealthConnectSync) {
                    try {
                        window.receiveHealthConnectSync(JSON.parse(payload));
                    } catch (_) {}
                }
            })();
        """.trimIndent()
    ...
    private fun isTrustedAppUri(uri: Uri): Boolean {
        val trustedHost = trustedAppUri.host ?: return false
        val host = uri.host ?: return false
        if (!uri.scheme.equals(trustedAppUri.scheme, ignoreCase = true)) return false
        if (!host.equals(trustedHost, ignoreCase = true)) return false
        val trustedPath = trustedAppUri.path.orEmpty().trimEnd('/')
        val currentPath = uri.path.orEmpty()
        return trustedPath.isEmpty() || currentPath.startsWith("$trustedPath/")
            || currentPath == trustedPath
    }
